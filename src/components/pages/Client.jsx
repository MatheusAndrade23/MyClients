...
function Client() {
  ...
  useEffect(() => {
    fetch(
      `https://myclients-backend.vercel.app/${process.env.REACT_APP_LINK}/${id}`,
      {
        method: "GET",
        headers: {
  ...
  function removeConta(_id, valor) {
    ...
    fetch(
      `https://myclients-backend.vercel.app/${process.env.REACT_APP_LINK}/${clientUpdated._id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className={styles.container}>
      {Client && (
        <div className={styles.dados}>
          <div className={styles.dados2}>
            ...
            {!showClientForm && (
              <>
                ...
                <p>
                  Conta Total: <span>{Client.total} R$</span>
                </p>
              </>
            )}
            {showClientForm && (
          ...
          <div>
            <button className={styles.btn} onClick={ShowEditar}>
              {!showClientForm ? "Editar" : "Cancelar"}
            </button>
          </div>
        </div>
      )}
      <div className={styles.adicionar}>
        <div className={styles.adicionarHeader}>
          ...
          <SubmitButton
            value={!showContasForm ? "Adicionar" : "Cancelar"}
            event={ShowContas}
          />
        </div>
        {showContasForm && (
          <ContaForm handleSubmit={createConta} contasData={Client} />
        )}
      </div>
